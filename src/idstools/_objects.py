    ...
    def update_data(self) -> pd.DataFrame:
        ...
        try:
            if not self.processed_data.empty: 
                common_features = list(set(self.features) & set(self.processed_data.columns.tolist()))
                new_features = list(set(self.processed_data.columns.tolist()) - set(self.features))

                self.features = common_features + new_features
                self.features.remove(self.label)
...
@use_decorator(emergency_logger)
class ValidationTarget(TargetData):
    """
    This class is used to load and share the validation data.

    Args:
        input_path (str): The path to the input data.
        input_delimiter (str): The delimiter of the input data.
        label (str): The label of the validation data.
        index (str): The index of the validation data.
        features (list[str]): The features of the validation data.
        output_path (str): The path to the output data.
        env_name (str): The name of the environment.
        step_name (str): The name of the step.
    ...
    Attributes:
        ...
        env_name (str): The name of the environment.
        step_name (str): The name of the step.
        data (pd.DataFrame): The validation data.
        filename (str): The filename of the input data.
        processed_data (pd.DataFrame): The processed data.
        analysis_results (dict): The analysis results.  
    """
    def __init__(self,
        ...
        logger.info("Initializing ValidationData object.")
        super().__init__(
            data=pd.DataFrame(),
            index=index,
            label=label,
